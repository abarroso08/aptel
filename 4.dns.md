...
dig abc.es SOA
SAME answer as before, no aa flag => no authoritative
...
dig domain NS    
dig domain MX    
dig domain SOA => <zone> SOA <MNAME> <RNAME> <serial> <refresh> <retry> <expire> <minimum>
...
### 2️⃣ Add the reverse zone to `named.conf`
...
### 3️⃣ Create the reverse zone file
...
### 6️⃣ Test reverse resolution with `dig`
