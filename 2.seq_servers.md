...
```
>Server not executing close().
We add close(fd) at the end of TCPechod
...
>In the netstat there is a FIN_WAIT(client stuck for a few seconds). Port 8888 used by that tcp connection so server cannot connect on reset
...
>Now we do not need to wait for client to definetly close, we can rebind to the same server port
```c
int yes=1;
